...
for file_meta in metadata:
	...
	if pr.attrib['procurement_status'] != "Completed" or er.attrib['procurement_status'] != "Completed" or pr.text == "Indeterminate" or er.text == "Indeterminate":
		continue

	tss = patient_data.find("{http://tcga.nci/bcr/xml/shared/2.7}tissue_source_site").text
...
fig, ax = plt.subplots()
all_centres = list(set(tss_pr_pos.keys()).union(set(tss_pr_neg.keys())).union(set(tss_er_pos.keys())).union(set(tss_er_neg.keys())))
BAR_WIDTH = 0.35

for i, c in enumerate(all_centres):
	p1 = ax.bar(i, tss_pr_pos[c], label="PR+", width=BAR_WIDTH, color="blue")
	p2 = ax.bar(i, tss_pr_neg[c], label="PR-", bottom=tss_pr_pos[c], width=BAR_WIDTH, color="orange")
	p3 = ax.bar(i + BAR_WIDTH, tss_er_pos[c], label="ER+", width=BAR_WIDTH, color="cyan")
	p4 = ax.bar(i + BAR_WIDTH, tss_er_neg[c], bottom=tss_er_pos[c], label="ER-", width=BAR_WIDTH, color="yellow")
