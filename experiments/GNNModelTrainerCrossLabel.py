...
class GNNModelTrainer:
    ...
    def train_and_validate(
        ...
        for suffix, train_policy, validation_policy in zip(
            (
                "_train_same_only_validiate_different_only",
                "_train_different_only_validiate_same_only",
            ),
            (eq, ne),
            (ne, eq),
        ):
            model_name = original_model_name + suffix
            # [ER scores, PR scores]
            ...
            for fold_num, (train_idxs, validation_idxs) in enumerate(folds):
                train_idxs = [
                    ...
                    for i in train_idxs
                    if train_policy(
                        self.dataset.data[i][2][0], self.dataset.data[i][2][1]
                    )
                ]
                # We use the train policy to decide training hyperparams e.g. early stopping
                train_validation_idxs = [
                    i
                ...
                model = LightningModel(
                    make_model(), self.er_pos_weight, self.pr_pos_weight
                )
                ...
                train_validation_loader = torch_geometric.loader.DataLoader(
                    torch.utils.data.Subset(self.dataset, train_validation_idxs),
                    batch_size=batch_size,
                )
                eval_validation_loader = torch_geometric.loader.DataLoader(
                    torch.utils.data.Subset(self.dataset, eval_validation_idxs),
                    batch_size=batch_size,
                )

                t0 = time()
                trainer.fit(model, train_loader, train_validation_loader)
                t1 = time()
                ...
                ax.plot(
                    ...
                    [
                        ...
                        if log["epoch"] == ckpt["epoch"]
                        and not pd.isna(log["val_er_loss"])
                    ],
                    marker="o",
                )
                fig.legend()
                ...
                ax.plot(
                    ...
                    [
                        ...
                        for log in logs.to_dict("records")
                        if log["epoch"] == ckpt["epoch"]
                        and not pd.isna(log["val_pr_loss"])
                    ],
                    marker="o",
                )
                fig.legend()
                ...
                with torch.no_grad():
                    for batch_data in eval_validation_loader:
                        y_pred.extend(
