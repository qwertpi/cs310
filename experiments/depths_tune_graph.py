...
for path in glob.glob("*/*_d*.metrics"):
    model_name, tail = path.split("/")[1].split("_")
    depth = int(tail.split("d")[1].split(".metrics")[0])
        ...
        for line in f.read().splitlines():
            ...
            if averages:
                if metric == "AUC_ROC_ER":
                    ...
                    means[model_name][depth] = (er_mean, pr_mean)
            if std_devs:
                if metric == "AUC_ROC_ER":
                    ...
                    stds[model_name][depth] = (er_std, pr_std)
...
for i, metric in enumerate(("AUC_ROC(ER)", "AUC_ROC(PR)")):
    ...
    for j, model in enumerate(("gnn", "gcn", "gat", "econv"), start=1):
        ...
        depths, depths_means, depths_stds = zip(
            *(
                (
                    (d, m, s)
                    for d, m, s in sorted(
                        ((d, m[i], model_stds[d][i]) for d, m in model_means.items()), key=lambda t: t[0]
                    )
                )
            )
        )
