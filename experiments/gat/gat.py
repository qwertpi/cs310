...
class Model(torch.nn.Module):
    def __init__(
        ...
        self.blocks = torch.nn.ModuleList(
            [GATBlock(act, num_heads, feat_dim, internal_dim)]
            + [
                GATBlock(act, num_heads, internal_dim, internal_dim)
...
if __name__ == "__main__":
    ...
    for width in tqdm([8, 16, 32, 64, 128, 256, 512, 1024]):
        trainer.train_and_validate(
            partial(Model, width),
            f"gat_w{int(log2(width))}",
        )
